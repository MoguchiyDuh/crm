...
class Project(Base):
    ...
    status_id: Mapped[int] = mapped_column(Integer, ForeignKey("project_statuses.id"), nullable=False)
    status: Mapped["ProjectStatus"] = relationship(back_populates="projects")  # noqa: F821

    priority_id: Mapped[int] = mapped_column(Integer, ForeignKey("project_priorities.id"), nullable=False)
    priority: Mapped["ProjectPriority"] = relationship(back_populates="projects")  # noqa: F821
    ...
    spent_hours: Mapped[int | None] = mapped_column(Integer, nullable=True)
